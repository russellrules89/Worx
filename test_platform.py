...
from app import app, corporate_crypto_payments, future_work_contracts, ledger_entries, submissions, token_issuances, worker_accounts


class TestWorkPlatform(unittest.TestCase):
    def setUp(self):
        app.config.update(TESTING=True, CONTRACT_ADMIN_API_KEY="test-contract-admin", OWNER_WALLET_ADDRESS="0x000000000000000000000000000000000000dEaD")
        ...
        corporate_crypto_payments.clear()
    ...
    def test_client_can_create_demo_task_with_allocation(self):
        ...
        self.assertFalse(response.get_json()["allocation"]["on_chain_payment_verified"])
    ...
    def test_corporate_crypto_payment_is_pending_until_admin_confirms(self):
        payment = self.client.post("/api/corporate-crypto-payments", json={"corporation_name": "Northstar Labs", "transaction_reference": "0xabc123", "asset": "USDC", "amount": "25.50"})
        self.assertEqual(payment.status_code, 201)
        payment_id = payment.get_json()["payment"]["id"]
        self.assertEqual(self.client.post(f"/api/corporate-crypto-payments/{payment_id}/confirm").status_code, 403)
        confirmed = self.client.post(f"/api/corporate-crypto-payments/{payment_id}/confirm", headers=self.contract_admin_headers)
        self.assertEqual(confirmed.status_code, 200)
        self.assertEqual(confirmed.get_json()["payment"]["status"], "confirmed")

    def test_corporate_crypto_payment_rejects_duplicate_transaction_reference(self):
        payload = {"corporation_name": "Northstar Labs", "transaction_reference": "0xabc123", "asset": "USDC", "amount": "25.50"}
        self.assertEqual(self.client.post("/api/corporate-crypto-payments", json=payload).status_code, 201)
        self.assertEqual(self.client.post("/api/corporate-crypto-payments", json=payload).status_code, 409)
